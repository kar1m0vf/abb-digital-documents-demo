...
export function getOrders(){const saved=read(ORDER_KEY,memoryOrders);return Array.isArray(saved)?saved.filter(safeOrder).slice(0,100).map(o=>{
  ...
  for(const id of selected){const raw=o.details?.[id]||{};const d={language:['en','az'].includes(raw.language)?raw.language:o.language,equivalent:raw.equivalent===true,equivalentCurrency:['EUR','USD','AZN'].includes(raw.equivalentCurrency)?raw.equivalentCurrency:'EUR',period:['1','3','6','12','custom'].includes(raw.period)?raw.period:'1',operation:['all','income','expense'].includes(raw.operation)?raw.operation:'all',start:typeof raw.start==='string'?raw.start:'',end:typeof raw.end==='string'?raw.end:''};if(d.period==='custom'&&!validRange(d.start,d.end))d.period='1';details[id]=d;}
...
export function accountDetail(id){return state.draft.details[id]??={language:state.draft.language,equivalent:false,equivalentCurrency:'EUR',period:'1',operation:'all',start:'',end:''};}
